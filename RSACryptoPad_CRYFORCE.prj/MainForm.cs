...
namespace RSACryptoPad
{
    public class MainForm : Form
    {
        ...
        private string versionString = " - RSACryptoPad [256 Kb] / CRYFORCE powered (build / 22.09.2013)";
        ...
        public MainForm(string fileName)
        {
            ...
            if(File.Exists(fileName))
            {
                ...
                streamReader.Close();
                Text = GetFileName(fileName) + versionString;
                cleanForm = true;
            }
        }

        public static int CurrentBitStrength { get; set; }
        ...
        private void InitializeComponent()
        {
            ...
            this.inputTextBox.Size = new System.Drawing.Size(740, 405);
        ...
        private string openFile(string title, string filterString)
        {
            ...
            if(openFileDialog.ShowDialog() == DialogResult.OK)
            {
                if(File.Exists(openFileDialog.FileName))
                {
                    ...
                    if(fileString != null)
                    {
                        Text = GetFileName(openFileDialog.FileName) + versionString;
        ...
        private bool saveFile()
        {
            ...
            if(saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    streamWriter.Close();
                    Text = GetFileName(saveFileDialog.FileName) + versionString;
        ...
        private void mainForm_Load(object sender, EventArgs e)
        {
            if(File.Exists("Settings.bin"))
            {
                ...
                wordWrapMenuItem.Checked = settings.Wrapping ? true : false;
            }
            Text = GetFileName(currentFileName) + versionString;
            inputTextBox.Focus();
            Resize += mainForm_Resize;
            Move += MainForm_Move;
            inputTextBox.Height = Size.Height - 53;
            inputTextBox.Width = Size.Width - 7;
        }
        ...
        private void inputTextBox_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if(clean)
            {
                if(e.Data.GetDataPresent(DataFormats.FileDrop, true))
                {
                    ...
                    var streamReader = new StreamReader(fileNames[0], true);
                    string fileContents = streamReader.ReadToEnd();
                    streamReader.Close();
                    currentFileName = fileNames[0];
                    Text = GetFileName(fileNames[0]) + versionString;
                    ...
                    case DialogResult.Yes:
                        {
                            if(saveFile())
                            {
                                ...
                                Text = "Untitled - RSACryptoPad [256 Kb] / CRYFORCE powered";
                                cleanForm = true;
                            }
                            break;
                        }
        ...
        private void saveMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    var streamWriter = new StreamWriter(currentFileName);
                    streamWriter.Write(inputTextBox.Text);
                    streamWriter.Flush();
                    streamWriter.Close();
                    cleanForm = true;
        ...
        private void rsaDecryptMenuItem_Click(object sender, EventArgs e)
        {
            if(inputTextBox.Text.Length != 0)
            {
                ...
                if(File.Exists(openFileDialog.FileName))
                {
                    ...
                    {
                        FinishedProcessDelegate finishedProcessDelegate = FinishedProcess;
                        UpdateTextDelegate updateTextDelegate = UpdateText;
                        try
                        {
                            var decryptionThread = new EncryptionThread();
                            var decryptThread = new Thread(decryptionThread.Decrypt);
