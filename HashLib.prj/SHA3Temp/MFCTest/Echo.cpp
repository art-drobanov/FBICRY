...
namespace Echo
{
    ...
    {
        memcpy(&m_state[8], a_data, m_buffer_size);
    ...
    {
        memcpy(&m_state[16], a_data, m_buffer_size);
    ...
    void EchoBase::TransformBytes(byte* a_data, int a_index, int a_length)
    {
        ...
        while (a_length > 0)
        {
            int length = m_buffer_size - m_buffer_pos;

            if (a_length >= length)
            {
                memcpy(&m_buffer[m_buffer_pos], a_data, length);
                m_processedBytes += length;
            ...
            {
                memcpy(&m_buffer[m_buffer_pos], a_data, a_length);
    ...
    void EchoBase::Finish()
    {
        ...
        if ((m_buffer_size - m_buffer_pos) <= 18)
        {
            int pad_len = m_buffer_size - m_buffer_pos;
            m_processedBytes -= pad_len;
            TransformBytes(m_pad, 0, m_buffer_size - m_buffer_pos);
            m_processedBytes += pad_len;
            memset(m_pad, 0, 192);
        } 

        int index = m_buffer_size - m_buffer_pos - 18;
