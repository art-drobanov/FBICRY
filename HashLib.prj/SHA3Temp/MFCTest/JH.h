...
namespace JH
{
    class JHHashSize
    {
        public:
            ...
            const static int HashSize224 = 28;
            const static int HashSize256 = 32;
            const static int HashSize384 = 48;
            const static int HashSize512 = 64;
    };

    const ulong initial_states[4][32] = 
    {
    ...
    class JH
    {
        private:
            ...
            void SS(int i, ulong cc0, ulong cc1)
            {
                m_state[12+i]  = ~m_state[12+i];                  
                m_state[14+i]  = ~m_state[14+i];                  
                m_state[0+i] ^= ((~(m_state[8+i])) & (cc0));      
                m_state[2+i] ^= ((~(m_state[10+i])) & (cc1));      
                ulong temp0 = (cc0) ^ ((m_state[0+i]) & (m_state[4+i]));
                ulong temp1 = (cc1) ^ ((m_state[2+i]) & (m_state[6+i]));
            ...
            void Swap(int i, ulong L, ulong R, int n)
            {
            ...
            {
                ulong* a_buffer = (ulong*)&a_data[a_index];

                for (int i = 0; i < 8; i++)  
                    m_state[i] ^= a_buffer[i];
        ...
        public:
            ...
            JH(int a_hashSize)
            {
                ASSERT((a_hashSize == JHHashSize::HashSize224) || 
                       (a_hashSize == JHHashSize::HashSize256) || 
                       (a_hashSize == JHHashSize::HashSize384) || 
                       (a_hashSize == JHHashSize::HashSize512));
