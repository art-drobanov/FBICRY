...
{
	class Murmur2 : MultipleTransformNonBlock, IHash32, IFastHashCodes, INonBlockHash
	{
		...
		public override HashResult ComputeBytes(byte[] a_data)
		{
			...
			while(length >= 4)
			{
				var k = (uint)(a_data[currentIndex++] | a_data[currentIndex++] << 8 | a_data[currentIndex++] << 16 | a_data[currentIndex++] << 24);
				k *= M;
				k ^= k >> R;
				k *= M;
		...
		{
			return ComputeInt(Converters.ConvertFloatToInt(a_data));
		}

		public override HashResult ComputeDouble(double a_data)
		{
			return ComputeLong(BitConverter.DoubleToInt64Bits(a_data));
		}
		...
		public override HashResult ComputeChars(char[] a_data)
		{
			...
			while(length >= 4)
			{
				uint k = a_data[currentIndex++] | ((uint)a_data[currentIndex++] << 16);
				k *= M;
				k ^= k >> R;
				k *= M;
		...
		public override HashResult ComputeShorts(short[] a_data)
		{
			...
			while(length >= 4)
			{
				var k = (uint)((ushort)a_data[currentIndex++] | (ushort)a_data[currentIndex++] << 16);
				k *= M;
				k ^= k >> R;
				k *= M;
		...
		public override HashResult ComputeUShorts(ushort[] a_data)
		{
			...
			while(length >= 4)
			{
				var k = (uint)(a_data[currentIndex++] | a_data[currentIndex++] << 16);
				k *= M;
				k ^= k >> R;
				k *= M;
		...
		public override HashResult ComputeInts(int[] a_data)
		{
			...
			while(length >= 4)
			{
				var k = (uint)a_data[currentIndex++];
				k *= M;
				k ^= k >> R;
				k *= M;
		...
		public override HashResult ComputeDoubles(double[] a_data)
		{
			...
			while(length >= 8)
			{
				long v = BitConverter.DoubleToInt64Bits(a_data[currentIndex++]);
		...
		public override HashResult ComputeFloats(float[] a_data)
		{
			...
			while(length >= 4)
			{
				int v = Converters.ConvertFloatToInt(a_data[currentIndex++]);
		...
		{
			return ComputeBytes(a_encoding.GetBytes(new[] {a_data}));
		}
