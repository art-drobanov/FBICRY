...
{
	class Murmur2_64 : MultipleTransformNonBlock, IHash64, INonBlockHash
	{
		...
		public override HashResult ComputeBytes(byte[] a_data)
		{
			...
			while(length >= 8)
			{
				ulong k = a_data[currentIndex++] | (ulong)a_data[currentIndex++] << 8 | (ulong)a_data[currentIndex++] << 16 |
			...
			switch(length)
			{
				...
				case 6:
					h ^= (ulong)a_data[currentIndex++] << 40 | (ulong)a_data[currentIndex++] << 32 | (ulong)a_data[currentIndex++] << 24 |
					     (ulong)a_data[currentIndex++] << 16 | (ulong)a_data[currentIndex++] << 8 | a_data[currentIndex++];
					h *= M;
					break;
				case 5:
					h ^= (ulong)a_data[currentIndex++] << 32 | (ulong)a_data[currentIndex++] << 24 | (ulong)a_data[currentIndex++] << 16 |
					     (ulong)a_data[currentIndex++] << 8 | a_data[currentIndex++];
					h *= M;
					break;
				case 4:
					...
					break;
				case 3:
					h ^= (ulong)a_data[currentIndex++] << 16 | (ulong)a_data[currentIndex++] << 8 | a_data[currentIndex++];
					h *= M;
					break;
				case 2:
					h ^= (ulong)a_data[currentIndex++] << 8 | a_data[currentIndex++];
					h *= M;
					break;
				case 1:
