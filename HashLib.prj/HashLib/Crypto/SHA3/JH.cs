...
namespace HashLib.Crypto.SHA3
{
	...
	abstract class JHBase : HashCryptoNotBuildIn
	{
		#region Consts
		...
		{
			ulong[] buffer = Converters.ConvertBytesToULongs(a_data, a_index, 64);
			...
			ulong m0 = m_state[0] ^ buffer[0];
			ulong m1 = m_state[1] ^ buffer[1];
			ulong m2 = m_state[2] ^ buffer[2];
			ulong m3 = m_state[3] ^ buffer[3];
			ulong m4 = m_state[4] ^ buffer[4];
			ulong m5 = m_state[5] ^ buffer[5];
			ulong m6 = m_state[6] ^ buffer[6];
			ulong m7 = m_state[7] ^ buffer[7];
			...
			m0 ^= m4 | m12;
			m2 ^= m6 | m14;
			m_state[12] = m12 ^ (m4 & m8) ^ buffer[4];
			m_state[14] = m14 ^ (m6 & m10) ^ buffer[6];
			m_state[4] = m4 ^ (temp2 & m0);

			m_state[6] = m6 ^ (temp3 & m2);
			m_state[8] = m8 ^ temp2 ^ buffer[0];
			m_state[10] = m10 ^ temp3 ^ buffer[2];
			...
			m1 ^= m5 | m13;
			m3 ^= m7 | m15;
			m_state[13] = m13 ^ (m5 & m9) ^ buffer[5];
			m_state[15] = m15 ^ (m7 & m11) ^ buffer[7];
			m_state[5] = m5 ^ (temp2 & m1);
			m_state[7] = m7 ^ (temp3 & m3);
			m_state[9] = m9 ^ temp2 ^ buffer[1];
			m_state[11] = m11 ^ temp3 ^ buffer[3];
		...
		protected override void Finish()
		{
			...
			if(m_processedBytes % 64 != 0)
				padindex += BlockSize - m_buffer.Pos;
		...
		public override void Initialize()
		{
			...
			{
				case 28:
					Array.Copy(initial_states[0], m_state, m_state.Length);
					break;
				case 32:
					Array.Copy(initial_states[1], m_state, m_state.Length);
					break;
				case 48:
					Array.Copy(initial_states[2], m_state, m_state.Length);
					break;
				case 64:
					Array.Copy(initial_states[3], m_state, m_state.Length);
					break;
			}
